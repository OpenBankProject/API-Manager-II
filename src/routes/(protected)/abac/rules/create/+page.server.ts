...
import { getCreateAbacRuleRoles } from "$lib/utils/roleChecker";
import { SessionOAuthHelper } from "$lib/oauth/sessionHelper";
import { createLogger } from "$lib/utils/logger";

const logger = createLogger("CreateAbacRuleServer");
...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const requiredRoles = getCreateAbacRuleRoles();

  logger.debug("Create ABAC rule page loaded");
